...
export function newControllerDecoratorWithInjector(
  family: Family,
  hook: (injector: Injector) => any,
  ...
  return function decorator(target: any, methodName?: string) {
    if (family === "contextual") {
      ...
      if (expressHooks) {
        console.log(
          `WARNING: contextual decorators should be specified after express hooks`
        );
      }
    }
    ...
    if (contextDef) {
      ...
      let contextDef2 =
        getMetadata(`contextDef:${family}`, target, methodName) || [];
