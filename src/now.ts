...
export interface ModuleData {
  ...
  controllerBindings: ((
    injector: Injector,
    controllerHooks: ModuleHooks,
    controllerContextDef: ModuleContextDef
  ) => { express: Router })[];
...
export class NowModule {
  ...
  constructor(data: ModuleData, parentModule?: NowModule) {
    ...
    data.controllerDecorators
      .reverse()
      .forEach((decorator) => decorator(FakeModule));
    const expressHooks: ExpressHook[] =
      Reflect.getMetadata("hooks:express", FakeModule) || [];
    const contextualHooks: ContextualHook[] =
      Reflect.getMetadata("hooks:contextual", FakeModule) || [];
    const expressContextDef: ExpressContextDef =
      Reflect.getMetadata("contextDef:express", FakeModule) || [];
    ...
    data.imports.forEach((imp) =>
      this.router.use(imp.path || "/", new NowModule(imp.module, this).router)
    );
  }
